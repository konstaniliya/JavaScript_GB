...
function move() {
    ...
    if (direction == 'x-') {
        ...
        new_unit = document.getElementsByClassName('cell-' + (coord_y + 1) + '-' + (coord_x))[0];
        if(new_unit == undefined){
            new_unit = document.getElementsByClassName('cell-' + (coord_y-19) + '-' + (coord_x + 20))[0];
        }
    }
    ...
    if (!isSnakeUnit(new_unit) && !new_unit.getAttribute('class').includes("obstacle-unit")) {
...
function renewScore(){
    var scoreUnit = document.getElementById("score");
    scoreUnit.innerHTML = "Your score is " + score;
}
